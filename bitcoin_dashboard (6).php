...
<div id="tableFilters" class="filters">
  ...
  <div class="filter-row">
    ...
    <label>Stochastic %K من:</label><input type="number" step="0.1" id="minK" placeholder="0">
...
// ==== Enhanced Filters: MACD + Period + Histogram + Stochastic & Filtered CSV ====
(function(){
  const table = document.getElementById('dataTable');
  if(!table) return;
  const tbody = table.querySelector('tbody');
  ...
  // Snapshot of original rows (immutable clone for rebuilds)
  const original = Array.from(tbody.querySelectorAll('tr')).map(tr => {
    const tds = tr.querySelectorAll('td');
    const getNum = (cell) => parseFloat(String(cell?.innerText || '').replace(/[^0-9.\-]/g,'')); 
    return {
  ...
  function applyPeriodToCharts(){
    ...
    if(sc){
      ...
      sc.data.datasets[0].data = sliceLastN(gK, period);
      sc.data.datasets[1].data = sliceLastN(gD, period);
      sc.update();
    }
  }

  // Override global CSV export to export **filtered rows (all pages)**
  window.downloadCSV = function(){
    const filtered = applyFilters();
    let csv = "day,price,MA7,MA30,BB_Up,BB_Mid,BB_Low,RSI,MACD,Signal,Hist,K,D
";
    filtered.forEach(o => {
      // Extract from DOM clone to preserve formatting
...
";
  ...
  // Export only the CURRENT page rows (visible in tbody after pagination render)
  window.downloadCSVPage = function(){
    const table = document.getElementById('dataTable');
    if(!table) return;
  ...
  // Listeners
  ['minPrice','maxPrice','minRSI','maxRSI','textSearch','minMACD','maxMACD','periodSelect','minHist','maxHist','minK','maxK','minD','maxD'].forEach(id=>{
    const el = document.getElementById(id); if(!el) return;
    el.addEventListener('input', ()=>{ currentPage=1; render(); });
    el.addEventListener('change', ()=>{ currentPage=1; render(); });
  });

  // Reset button keeps working (already added earlier)
  const reset = document.getElementById('resetFilters');
  if(reset){
    reset.addEventListener('click', ()=>{
...
// ==== Extend Filters with Histogram + Stochastic ====
(function(){
  const table = document.getElementById('dataTable');
  if(!table) return;
  const tbody = table.querySelector('tbody');
  ...
  function applyFilters(){
    const minH = parseFloat(document.getElementById('minHist')?.value || '');
    const maxH = parseFloat(document.getElementById('maxHist')?.value || '');
    const minK = parseFloat(document.getElementById('minK')?.value || '');
    const maxK = parseFloat(document.getElementById('maxK')?.value || '');
    const minD = parseFloat(document.getElementById('minD')?.value || '');
    const maxD = parseFloat(document.getElementById('maxD')?.value || '');

    return original.filter(o => {
      if(!isNaN(minH) && !(o.hist >= minH)) return false;
      if(!isNaN(maxH) && !(o.hist <= maxH)) return false;
      if(!isNaN(minK) && !(o.k >= minK)) return false;
      if(!isNaN(maxK) && !(o.k <= maxK)) return false;
      if(!isNaN(minD) && !(o.d >= minD)) return false;
      if(!isNaN(maxD) && !(o.d <= maxD)) return false;
      return true;
    });
  }
  ...
  function render(){
    ...
    const totalPages = Math.max(1, Math.ceil(filtered.length / perPage));
    tbody.innerHTML = '';
    filtered.slice(0, perPage).forEach(o => tbody.appendChild(o.row.cloneNode(true)));
  }

  ['minHist','maxHist','minK','maxK','minD','maxD'].forEach(id=>{
    const el = document.getElementById(id); if(!el) return;
...
// Export only current page's rows
window.downloadCSVPage = function(){
  const rows = Array.from(document.querySelectorAll('#dataTable tbody tr'));
  if(rows.length === 0){ alert('لا يوجد بيانات في هذه الصفحة'); return; }
  let csv = "day,price,MA7,MA30,BB_Up,BB_Mid,BB_Low,RSI,MACD,Signal,Hist,K,D\n";
  rows.forEach(tr => {
    const tds = tr.querySelectorAll('td');
    const row = Array.from(tds).map(td => td.innerText.replace(/\$/g,''));
